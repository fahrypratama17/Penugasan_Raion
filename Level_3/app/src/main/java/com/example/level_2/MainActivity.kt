...
import com.example.level_2.view.SongDetailPage
...
class MainActivity : ComponentActivity() {
    ...
    fun navigation(navController: NavHostController) {
        ...
        NavHost(navController = navController, startDestination = "LoginPage") {
            ...
            composable("SongDetail/{judul}/{penyanyi}/{deskripsi}/{type}") { backStackEntry ->
                val judul = backStackEntry.arguments?.getString("judul") ?: ""
                val penyanyi = backStackEntry.arguments?.getString("penyanyi") ?: ""
                val deskripsi = backStackEntry.arguments?.getString("deskripsi") ?: ""
                val type = backStackEntry.arguments?.getString("type") ?: ""
                SongDetailPage(judul, penyanyi, deskripsi, type, navController )
            }
        }
    }
}